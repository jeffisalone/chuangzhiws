...
The auth routes are mounted under `/auth`:

- `GET /auth/public-key` returns the RSA public key for encrypted login/register payloads.
- `POST /auth/register` creates a user in the D1 `users` table.
- `POST /auth/login` verifies the password and writes an HTTP-only `session` cookie.

The frontend encrypts auth form data with AES-GCM and encrypts the AES key with RSA-OAEP SHA-256. In production, configure both `AUTH_PRIVATE_KEY` and `AUTH_PUBLIC_KEY` in the Worker environment. If they are omitted, the worker generates an in-memory key pair for local development only.

The `0002_add_users_table.sql` migration creates the `users` and `sessions` tables for the D1 binding named `DB`, which is configured to use the `jljy` database in `wrangler.jsonc`.
